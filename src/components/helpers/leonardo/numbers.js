...
export class Vizualizer extends React.Component {
    ...
    return (
      <div className="controls">
        <div className="item-toggle">
          Showing: Leonardo{' '}
          <span
            data-selected={this.state.showing == 'number'}
            onClick={() => this.setState({ showing: 'number' })}
          >
            numbers
          </span>{' '}
          /{' '}
          <span
            data-selected={this.state.showing == 'indexes'}
            onClick={() => this.setState({ showing: 'indexes' })}
          >
            indices
          </span>
        </div>
        <div className="n">
          ...
          <input
            ...
            value={this.state.n}
            onChange={evt =>
              this.setState({ n: Math.min(parseInt(evt.target.value), 1000) })
            }
          />
        </div>
      </div>
  ...
  render() {
    ...
    const isLeonardo = Object.assign(
      {},
      ...Array.from(leonardoNumbers, k => ({ [k]: true }))
    );
        ...
        <div className="scroll-container">
          {range(1, n + 1).map(i => (
            <div key={i} className="row">
              <span
                style={{
                  background: isLeonardo[i]
                    ? color(i == 1 ? 1 : leonardoIndex[i])
                    : 'inherit',
                }}
                className="index"
              >
...
const color = n => {
  const color = Math.floor(
    Math.abs(Math.sin(15 + n) * 16777215) % 16777215
  ).toString(16);
